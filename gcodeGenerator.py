...
class gcodeGenerator(object):
        ...
        def __init__(self):
                ...
                self.__tools=[]
#                self.__machine_size = [320.0, 250.0, 300.0, float("inf")] # machine_size in mm
...
#                    self.comment("arclength %0.4f" % arclength)
#                    self.comment("arcangle %0.4f" % arcangle)
                    self._eValue += arclength * ePerMM
#                    cmd += "var%0.4f " % arclength
                    cmd += "E%0.4f F%d" % (self._eValue, self._feedPrint)
                    self._list += [cmd]
